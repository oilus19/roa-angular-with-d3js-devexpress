...
var workbook = XLSX.readFile('app/assets/xlsx/data.xlsx');
...
exports.performance_dataset = function(req, res){
    ...
    workbook.SheetNames.forEach(function(sheetName) {
        ...
        for (var i = 0; i < json_data.length; i++) {
        	if (user_type_array.indexOf(json_data[i]["usertype"])==-1)
        	{
        		continue;
        	}
        	if(!json_data[i]["client"])
        		continue;
			...
			if (user_type_array.indexOf(json_data[i]["usertype"])!=-1) {
				if(temp_object["length"] == undefined)
				{
					...
					temp_object["roi"] = 0;
					temp_object["ads"] = 0;
					temp_object["visitors"] = 0;
					temp_object["newvisitors"] = 0;
				}
				...
				temp_object["cpu"] += Number(json_data[i]["cpu"]) || 0;
				temp_object["cpnu"] += Number(json_data[i]["cpnu"])|| 0;
				...
				temp_object["ads"] ++;
				if(json_data[i]["usertype"] == "New Visitor")
					temp_object["newvisitors"]++;
				if(json_data[i]["usertype"] == "Returning Visitor")
					temp_object["visitors"]++;
			}
        };
        res.json(result);
    });
...
exports.planner_dataset = function(req, res){
    ...
    workbook.SheetNames.forEach(function(sheetName) {
        ...
        for (var i = 0; i < json_data.length; i+=10) {
        	if (limit == 0)
        			break;
        	if(json_data[i]['network'])
        	{
        		result.push(json_data[i]);
        		if(network_list.indexOf(json_data[i]["network"]) == -1)
        			network_list.push(json_data[i]["network"]);
        		cost = (json_data[i]["cost"] || "");
        		sum_budget += Number(cost.replace(",",".")) || 0;
        		sum_impression += Number(json_data[i]["impacts"]);
        		limit--;
        	}
        }
...
exports.dashboard_dataset = function(req, res){
    ...
    workbook.SheetNames.forEach(function(sheetName) {
        ...
        for (var i = 0; i < json_data.length; i++) {
        	...
        	if(json_data[i]['network'])
        	{
        		var date = Math.round(Number(json_data[i]["datetime"].split("/")[0]))-1;

        		result.push(json_data[i]);
        		revenue += Number(json_data[i]["revenue"]) || 0;
        		if(!daily_revenue_list[date])
        			daily_revenue_list[date] = 0;
        		daily_revenue_list[date] += revenue;
        		avg_roi += Number(json_data[i]["roi"]) || 0;
        		avg_cpc += Number(json_data[i]["cpc1"]) || 0;
        		limit--;
        	}
        }
        avg_roi /= keep_limit;
...
exports.roi_dataset = function(req, res){
    ...
    var combine_name = req.body.combine_name;
    var user_type_array = req.body.user_type_array;
    var result = {"name" : "Cost", "children" : [], "value" : 0, "ads" : 0, "cpu" : 0, "revenue" : 0};
    ...
    workbook.SheetNames.forEach(function(sheetName) {
        ...
        for (var i = 0; i < json_data.length; i++) {
			...
			if (user_type_array.indexOf(json_data[i]["usertype"])!=-1) {
                ...
                temp_object_parent["cpu"] += Number(json_data[i]["cpu"]) || 0;
                ...
                result["ads"] ++;
                result["cpu"] += Number(json_data[i]["cpu"]) || 0;
...
exports.breakdown_dataset = function(req, res){
    ...
    workbook.SheetNames.forEach(function(sheetName) {
        ...
        for (var i = 0; i < json_data.length; i+=30) {
            ...
            {
                result.push(json_data[i]);
                if(filter_list.daypart.indexOf(json_data[i]["daypart"]) == -1)
                    filter_list.daypart.push(json_data[i]["daypart"]);
                ...
                if(filter_list['length'].indexOf(json_data[i]["length"]) == -1)
                    filter_list['length'].push(json_data[i]["length"]);
                ...
                if(filter_list.network.indexOf(json_data[i]["network"]) == -1)
                    filter_list.network.push(json_data[i]["network"]);
                if(filter_list.channel.indexOf(json_data[i]["channel"]) == -1)
                    filter_list.channel.push(json_data[i]["channel"]);
                limit--;
            }
        }
